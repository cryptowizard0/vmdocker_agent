...
# Make Node.js honour HTTPS_PROXY/HTTP_PROXY env vars that Docker Sandbox injects.
# Without this flag Node's built-in https module ignores those variables and
# tries to connect directly, which fails in sandbox environments that only
# allow outbound traffic through the proxy.
