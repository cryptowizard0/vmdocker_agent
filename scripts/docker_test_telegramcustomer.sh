...
IMAGE_NAME="${IMAGE_NAME:-chriswebber/docker-telegramcustomer:latest}"
CONTAINER_NAME="${CONTAINER_NAME:-hymatrix-telegramcustomer-test}"
RESTORE_CONTAINER_NAME="${RESTORE_CONTAINER_NAME:-hymatrix-telegramcustomer-test-restore}"
HOST="${HOST:-127.0.0.1}"
PORT="${PORT:-8080}"
...
WORKSPACE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/vmdocker-telegramcustomer-smoke.XXXXXX")"
...
if payload.get("format") != "telegramcustomer.runtime.v1":
...
start_container() {
  ...
  docker run --name "${container_name}" -d \
    ...
    -e RUNTIME_TYPE=telegramcustomer \
...
echo "[INFO] starting telegramcustomer container: ${CONTAINER_NAME}"
start_container "${CONTAINER_NAME}"

echo "[INFO] calling /vmm/spawn"
curl -fsS -X POST "http://${HOST}:${PORT}/vmm/spawn" \
  -H 'Content-Type: application/json' \
  -d '{"Pid":"telegramcustomer-pid","Owner":"owner-1","CuAddr":"cu-1","Evn":{},"Tags":[]}' > "${WORKSPACE_DIR}/spawn.json"
...
echo "[OK] telegramcustomer smoke test passed"
